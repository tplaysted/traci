...
sumoCmd = [sumoBinary, "-c", "sumo/cross.sumocfg", "-b", "0"]


def eval_genomes(genomes, config, runs_per_net=2):
...
def eval_genomes_auxiliary(genomes, config, array, runs_per_net=1):
    ...
    for j in range(len(array)):
        array[j] = min(fitnesses[j])  # get worst score, write to shared array
...
def run(config_file):
    ...
    winner = p.run(eval_genomes_parallel, 25)
